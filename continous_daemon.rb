...
loop do
  pid = Process.fork do
    ...
    select.each do |previous_thread|
      ...
      if thread.poleman
        adapter.execute("UPDATE poles SET status='', poleman='#{thread.poleman}', pole_time='#{thread.pole_time}', op_time='#{thread.created_time}' WHERE id_thread = #{previous_thread.id_thread}")
      end
    end
  end
